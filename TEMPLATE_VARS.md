...
|----------|---------|-------------|---------|
| `{{PROJECT_NAME}}` | `cashflow-buddy` | Project identifier (kebab-case) | scaffolding only |
| `{{SERVER_SLUG}}` | `cashflow-buddy-mcp-server` | Server/repo name (kebab-case) | package.json, config.ts, vite.config.ts |
| `{{DISPLAY_NAME}}` | `Cashflow Buddy` | Human-readable project name | package.json, server.ts, TOOLS_OVERVIEW.md, Dockerfile, README |
...
| `{{DEV_PORT}}` | `3000` | Local development port | .env.example, README.md |
| `{{GITHUB_OWNER}}` | `RobertvandeCamp` | GitHub repo owner | scaffolding only |
| `{{ECR_REPO_NAME}}` | `cashflow-buddy/cashflow-buddy-mcp-server` | ECR repository path | .github/workflows/build-test-deploy.yml, docker/Dockerfile |
